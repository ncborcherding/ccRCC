...
```{r}
Tcells <- subset(integrated, Minor == "CD8.T.Cell" | Minor == "Act.T.Cell")
...
for (i in 1:length(T.list)) {
    T.list[[i]] <- SCTransform(T.list[[i]], verbose = FALSE, conserve.memory=TRUE, vars.to.regress = c("S.Score", "G2M.Score"))
}
rm(Tcells)
```
...
rm(integrated2)
DimPlot(object = integrated2, reduction = 'umap', label = T, group.by = "cloneType") 
```
...
Tcells <- RunUMAP(object = Tcells, reduction = "pca", 
    dims = 1:30)
Tcells <- FindNeighbors(object = Tcells, dims = 1:30, force.recalc = T)
...
Tcells <- subset(Tcells, idents = c(0,1,2,3,4,5,6,8))

names <- Tcells[[]]
names <- rownames(subset(names, newCluster == 5))
Idents(Tcells, cells = names) <-0

names <- Tcells[[]]
names <- rownames(subset(names, newCluster == 8))
Idents(Tcells, cells = names) <-0

names <- Tcells[[]]
names <- rownames(subset(names, newCluster == 6))
Idents(Tcells, cells = names) <-5


Tcells@active.ident <- factor(Tcells@active.ident, levels = c(0,1,2,3,4,5))
Tcells@meta.data$newCluster <- Tcells@active.ident


outlier <- as.data.frame(Tcells@reductions$umap@cell.embeddings)
outlier <- rownames(outlier[outlier$UMAP_1 < -3.9 & outlier$UMAP_2 > 4,])
names <- rownames(Tcells@reductions$umap@cell.embeddings)
`%!in%` = Negate(`%in%`)
names <- names[names %!in% outlier]
Tcells <- subset(Tcells, cells = names)
...
ggsave(path = "./DataAnalysis/CellType/CD8.Tcells", filename="IntegratedObject_byType.eps", width=3.75, height=3)
...
ggsave(path = "DataAnalysis/CellType/CD8.Tcells/markers", file = "Top10_dotplot.eps", height=10, width=3)
```
...
file_list <- list.files("./DataAnalysis/CellType/CD8.Tcells/DGE/")
...
sds <- slingshot(Embeddings(Tcells, "umap"), clusterLabels = Tcells@active.ident, allow.breaks = TRUE, stretch = 0, reducedDim = "UMAP",end.clus = 3) #Calcualting the trajectory
```
...
write.table(output, file = "DataAnalysis/CellType/CD8.Tcells/slingshot/VarImp_model.txt", col.names=NA, sep="\t",append=F)
```
...
ggsave("./DataAnalysis/CellType/CD8.Tcells/slingshot/top20_varImp_raw.pdf")
...
ggplot(subset_out, aes(x=reorder(X, Var.Imp), y=Var.Imp)) +
    ...
    coord_flip() + 
    theme(axis.title.y = element_blank())
ggsave("./DataAnalysis/CellType/CD8.Tcells/slingshot/top20_varImp_scaled.pdf")
```
...
ggplot(subset_out, aes(x=reorder(X, Var.Imp), y=Var.Imp)) +
    ...
    coord_flip() + 
    theme(axis.title.y = element_blank())
ggsave("./DataAnalysis/CellType/CD8.Tcells/slingshot/Filtered_varImp_raw.pdf")
...
ggplot(subset_out, aes(x=reorder(X, Var.Imp), y=Var.Imp)) +
    ...
    coord_flip() + 
    theme(axis.title.y = element_blank())
ggsave("./DataAnalysis/CellType/CD8.Tcells/slingshot/Filtered_varImp_scaled.pdf")
```
...
ggsave("DataAnalysis/CellType/CD8.Tcells/clonotype/Clonal_overlap_morisita.pdf", height=3, width=4)

clonalOverlap(CD8, cloneCall = "gene+nt", method = "overlap")
ggsave("DataAnalysis/CellType/CD8.Tcells/clonotype/Clonal_overlap_overlap.pdf", height=3, width=4)
...
ggsave("DataAnalysis/CellType/CD8.Tcells/clonotype/Clonal_overlap_morisita_TumorOnly.pdf", height=3, width=4)

clonalOverlap(TumorOnly, cloneCall = "gene+nt", method = "overlap")
ggsave("DataAnalysis/CellType/CD8.Tcells/clonotype/Clonal_overlap_overlap_TumorOnly.pdf", height=3, width=4)
```
...
```{r}
load("./DataAnalysis/CellType/CD8.Tcells/GSEA/H_enrichment.rda")
meta <- Tcells[[]]
...
for (i in seq_along(top10)){
  ...
  ggsave(path = "DataAnalysis/CellType/CD8.Tcells/GSEA/", file = paste0("Ridge_ClusterTop10_TypevsCluster_", top10[i], ".pdf"), height=3, width=5)
}

for (i in seq_along(top10)){
  ridgeEnrichment(meta, gene.set = top10[i], group = "newCluster", add.rug = TRUE) 
  ggsave(path = "DataAnalysis/CellType/CD8.Tcells/GSEA/", file = paste0("Ridge_ClusterTop10_Cluster_", top10[i], ".pdf"), height=3, width=5)
}
...
for (i in seq_along(top10)){
  ...
  ggsave(path = "DataAnalysis/CellType/CD8.Tcells/GSEA/", file = paste0("Ridge_ClusterTop10_Type_", top10[i], ".pdf"), height=3, width=5)
}
