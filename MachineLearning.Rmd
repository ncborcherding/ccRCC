...
for (i in seq_along(survival)) {
    ...
    for (j in seq_along(gene_list)) {
        ...
        rctrl1 <- rfeControl(returnResamp = "all", functions = caretFuncs, saveDetails = TRUE, method="repeatedcv", number=10, repeats=128)
        #Creating the model
        model <- rfe(as.factor(tmp$OS) ~ ., data = as.matrix(tmp),
...
```{r}
dfTPM <- dfTPM[, colnames(dfTPM) %in% genes]
```
